...
struct GroupActivityStateStorage {
    let activity: ActivityType = .group
    var subcategory: SubcategoryType?
    var trainerName: String?
    var date: Date?
}

class GroupActivityCoordinator: BaseCoordinator, Coordinator {

    var onFinish: (() -> Void)?

    private unowned let navigationController: UINavigationController
    private unowned let appointmentsManager: AppointmentManager
    private var stateStorage = GroupActivityStateStorage()
    ...
    private func showChooseActivityTypeScreen() {
        ...
        vc.onSelected = { [weak self] (activity: String) in
            guard let subcategory = SubcategoryType(rawValue: activity) else {
                fatalError("Unknown fitness activity type")
            }
            self?.stateStorage.subcategory = subcategory
            if let trainerName = subcategory.getTrainers().first {
